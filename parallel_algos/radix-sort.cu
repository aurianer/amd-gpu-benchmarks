...
int main(int argc, char** argv)
{
    ...
    std::printf("radix sort normal time for %zu key-value pairs: %f s, bandwidth: %f MiB/s\n",
        numKeys, timeRadixSort / 1000,
        float(numBytesMoved) / (timeRadixSort / 1000) / (1024 * 1024));
    std::printf(
        "radix sort with memory tracking time for %zu key-value pairs: %f s, bandwidth: %f MiB/s\n",
        numKeys, timeRadixSortTracked / 1000,
        float(numBytesMoved) / (timeRadixSortTracked / 1000) / (1024 * 1024));
